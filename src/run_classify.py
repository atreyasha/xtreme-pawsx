...
from .processor_pawsx import PawsxProcessor
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if output_file:
            ...
            with open(output_file, 'w') as fout:
                ...
                sentences = [[w for w in s if w != pad_token_id]
                             for s in sentences]
                sentences = [
                    tokenizer.convert_ids_to_tokens(s) for s in sentences
                ]
                #fout.write('Prediction\tLabel\tSentences\n')
                for p, l, s in zip(list(preds), list(out_label_ids),
                                   sentences):
                    s = ' '.join(s)
                    if label_list:
                        p = label_list[p]
...
def load_and_cache_examples(args,
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if split == 'train':
            ...
            examples = processor.get_translate_test_examples(
                args.data_dir, language)
        elif split == 'dev':
            examples = processor.get_dev_examples(args.data_dir, language)
        elif split == 'pseudo_test':
            examples = processor.get_pseudo_test_examples(
                args.data_dir, language)
        else:
    ...
    if output_mode == "classification":
        ...
        raise ValueError("No other `output_mode` for {}.".format(
            args.task_name))

    if args.model_type == 'xlm':
        all_langs = torch.tensor([f.langs for f in features], dtype=torch.long)
        dataset = TensorDataset(all_input_ids, all_attention_mask,
                                all_token_type_ids, all_labels, all_langs)
    else:
        dataset = TensorDataset(all_input_ids, all_attention_mask,
...
def main():
    ...
    if args.do_train:
        if args.init_checkpoint:
            ...
            logger.info("loading from existing model {}".format(
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        with open(output_eval_file, 'w') as writer:
            for key, value in results.items():
                writer.write('{} = {}\n'.format(key, value))
            writer.write("Best checkpoint is {}, best accuracy is {}".format(
                best_checkpoint, best_score))
            logger.info("Best checkpoint is {}, best accuracy is {}".format(
                best_checkpoint, best_score))
